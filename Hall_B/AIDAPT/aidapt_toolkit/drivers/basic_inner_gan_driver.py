...
def run(config):
    ...
    vertex_parser = aidapt_toolkit.data_parsers.make(
        "aidapt_numpy_reader_v0", config=config["vertex_parser"], name="vertex_parser"
    )
    detector_parser = aidapt_toolkit.data_parsers.make(
        "aidapt_numpy_reader_v0",
        config=config["detector_parser"],
        name="detector_parser",
    )
    lab2inv_prep = aidapt_toolkit.data_prep.make(
        "lab_variables_to_invariants", config=config["lab2inv"]
    )
    d_scaler = aidapt_toolkit.data_prep.make(
        "numpy_minmax_scaler", config=config["d_scaler"], name="detector_scaler"
    )
    v_scaler = aidapt_toolkit.data_prep.make(
        "numpy_minmax_scaler", config=config["v_scaler"], name="vertex_scaler"
    )
    model = aidapt_toolkit.models.make(
        "tf_cgan_v0", config=config["model"], name="mlp_gan_model"
    )
    ...
    fig.savefig(
        os.path.join(config["driver"]["save_path"], "reconstruction_errors.png")
    )
