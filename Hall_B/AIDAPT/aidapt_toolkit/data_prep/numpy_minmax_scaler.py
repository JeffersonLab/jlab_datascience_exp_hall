...
class NumpyMinMaxScaler(JDSTDataPrep):
    """Module performs a min/max scaling on numpy ndarrays.
    ...
    def train(self, data: np.ndarray):
        ...
        self.data_range = np.where(
            self.data_range < 1e-5, np.ones_like(self.data_range), self.data_range
        )
    ...
    def run(self, data: np.ndarray):
        ...
        data_scaled = data_zero_one * (self.feature_range[1] - self.feature_range[0])
