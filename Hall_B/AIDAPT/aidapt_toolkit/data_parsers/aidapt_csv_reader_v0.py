...
class AIDAPTCSVReaderV0(JDSTDataParser):
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        for file in self.config["filepaths"]:
            ...
            data = pd.read_csv(file, sep=self.config["sep"], **self.config["read_args"])
        ...
        output = pd.concat(
            data_list, axis=self.config["axis"], **self.config["concat_args"]
        )
        return output

    # Unimplemented functions below
    def save_data(self, path: str):
        aidapt_numpy_reader_log.warning("save_data() is currently unimplemented.")
        pass

    def load_config(self, path: str):
        aidapt_numpy_reader_log.debug(
            "load_config() is currently unimplemented. " "Calling load()..."
        )
        return self.load(path)

    def save_config(self, path: str):
        aidapt_numpy_reader_log.debug(
            "save_config() is currently unimplemented." " Calling save()...?"
        )
