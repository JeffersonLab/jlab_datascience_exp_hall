...
class PunziNetLoss(object):
    ...
    def compute_sparse_matrices(self,sigma_components):
        ...
        sig_m_range.to(self.device)

        idx_range = (1 + range_idx_high - range_idx_low).byte()
        idx_range.to(self.device)
        v1, v2, v1_bkg, v2_bkg = ([], [], [], [])
        ...
        for j in range(idx_range.max()):
           ...
           v1.append(range_idx_low[indices].to(self.device) + j)
           v2.append(indices)

           indices_bkg = torch.nonzero((idx_range > j) & bkg)[:, 0]
           indices_bkg.to(self.device)
           v1_bkg.append(range_idx_low[indices_bkg].to(self.device) + j)
           v2_bkg.append(indices_bkg)
        #+++++++++++++++++++++++++++++++++++++++++++++++

        i = torch.zeros(2, idx_range.sum(), dtype=torch.long,device=self.device)
        i[0, :] = torch.cat(v1).to(self.device)
        i[1, :] = torch.cat(v2).to(self.device)
        v = torch.ByteTensor([1]).expand(i.shape[1]).to(self.device)

        _, inverse_indices = torch.unique(gen_mass[sig_m_range], sorted=True, return_inverse=True)
        inverse_indices.to(self.device)
        i_sig = torch.zeros(2, len(inverse_indices), dtype=torch.long,device=self.device)
        i_sig[0, :] = inverse_indices
        i_sig[1, :] = torch.arange(len(gen_mass))[sig_m_range].to(self.device)
        v_sig = torch.ByteTensor([1]).expand(i_sig.shape[1]).to(self.device)

        i_bkg = torch.zeros(2, (idx_range * bkg).sum(), dtype=torch.long,device=self.device)
        i_bkg[0, :] = torch.cat(v1_bkg).to(self.device)
        i_bkg[1, :] = torch.cat(v2_bkg).to(self.device)
        v_bkg = torch.ByteTensor([1]).expand(i_bkg.shape[1]).to(self.device)

        sparse_shape = torch.Size([self.n_mass_hypotheses, len(gen_mass)])
        in_range = torch.sparse.ByteTensor(i, v, sparse_shape).to(self.device)
        sig_sparse = torch.sparse.ByteTensor(i_sig, v_sig, sparse_shape).to(self.device) * in_range
        bkg_sparse = torch.sparse.ByteTensor(i_bkg, v_bkg, sparse_shape).to(self.device)
