...
class PunziNet(torch.nn.Module,JDSTModel):
    ...
    def __init__(self,path_to_cfg,user_config={}):
        ...
        output_weight_initializer = self.config['output_weight_initializers']
        output_bias_initializer = self.config['output_bias_initializers']
        ...
        torch_optimizer = TorchOptimizers()

        # BCE: 
        bce_optimizer_str = self.config['bce_optimizer']
        bce_learning_rate = self.config['bce_learning_rate']
        bce_lr_scheduler_mode = self.config['bce_scheduler_mode']
        bce_lr_scheduler_factor = self.config['bce_scheduler_factor']
        bce_lr_scheduler_patience = self.config['bce_scheduler_patience']
        ...
        self.bce_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.bce_optimizer,bce_lr_scheduler_mode,bce_lr_scheduler_factor,bce_lr_scheduler_patience)

        # Punzi:
        punzi_optimizer_str = self.config['punzi_optimizer']
        punzi_learning_rate = self.config['punzi_learning_rate']
        punzi_lr_scheduler_mode = self.config['punzi_scheduler_mode']
        punzi_lr_scheduler_factor = self.config['punzi_scheduler_factor']
        punzi_lr_scheduler_patience = self.config['punzi_scheduler_patience']
        ...
        # Set up the learning rate scheduler, as it was suggested in the punzi paper:
        self.punzi_lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.punzi_optimizer,punzi_lr_scheduler_mode,punzi_lr_scheduler_factor,punzi_lr_scheduler_patience)
        ...
        # BCE:
        self.n_epochs_bce = self.configp['n_epochs_bce']
        self.read_epochs_bce = self.configp['read_epochs_bce']
        self.mon_epochs_bce = self.configp['mon_epochs_bce']
        self.snapshot_epochs_bce = self.configp['snapshot_epochs_bce']
        self.batch_size_bce = self.configp['batch_size_bce']

        # Punzi:
        self.n_epochs_punzi = self.configp['n_epochs_punzi']
        self.read_epochs_punzi = self.configp['read_epochs_punzi']
        self.mon_epochs_punzi = self.configp['mon_epochs_punzi']
        self.snapshot_epochs_punzi = self.configp['snapshot_epochs_punzi']
        self.batch_size_punzi = self.configp['batch_size_punzi']
    #*********************************************
    ...
    #*********************************************
    def train(self,x,y,w,s,x_test,y_test,w_test,s_test):
